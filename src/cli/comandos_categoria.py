...
def listar_categorias(tipo: Optional[str]):
    ...
    if not categorias:
        click.echo("[INFO] Nenhuma categoria cadastrada.")
        return
...
@click.option("--descricao", "-d", help="Descricao da categoria")
def adicionar_categoria(nome: str, tipo: str, limite: Optional[float], descricao: Optional[str]):
    ...
    try:
        ...
        click.echo(f"[OK] Categoria '{categoria.nome}' criada com sucesso!")
...
@click.option("--descricao", "-d", help="Nova descricao")
def editar_categoria(nome_atual: str, tipo: str, novo_nome: Optional[str], limite: Optional[float], descricao: Optional[str]):
    ...
    if not categoria:
        click.echo(f"[ERRO] Categoria '{nome_atual}' nao encontrada.", err=True)
        return
    ...
    try:
        ...
        click.echo("[OK] Categoria editada com sucesso!")
    except ValueError as e:
        click.echo(f"[ERRO] {e}", err=True)
...
def excluir_categoria(nome: str, tipo: str):
    ...
    if not categoria:
        click.echo(f"[ERRO] Categoria '{nome}' nao encontrada.", err=True)
        return

    try:
        gerenciador.excluir_categoria(categoria.id)
        click.echo(f"[OK] Categoria '{nome}' excluida com sucesso!")
    except ValueError as e:
        click.echo(f"[ERRO] {e}", err=True)
